...
void BackgroundSlicingProcess::process_fff()
{
	...
	if (this->set_step_started(bspsGCodeFinalize)) {
	    if (! m_export_path.empty()) {
			...
			{
				std::string err_msg = "Copying of the temporary G-code has finnished but the original code couldnt be opened during copy check. The output G-code is at " + export_path + ".tmp.";
				throw std::runtime_error(_utf8(L(err_msg)));
			}
			else if (with_check && copy_ret_val == -5)
			{
				std::string err_msg = "Copying of the temporary G-code has finnished but the exported code couldnt be opened during copy check. The output G-code is at " + export_path + ".tmp.";
				throw std::runtime_error(_utf8(L(err_msg)));
			}
			else if (with_check && copy_ret_val == -6)
			{
				std::string err_msg = "Copying of the temporary G - code has finnished but the exported file has different legth than the original. The corrupted output G-code is at " + export_path + ".tmp.";
				throw std::runtime_error(_utf8(L(err_msg)));
			}
